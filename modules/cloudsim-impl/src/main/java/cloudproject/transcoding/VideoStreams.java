...
public class VideoStreams implements Callable<String> {
	...
	// video Id
	private int videoId;
	// The number of cloudlets are gonna be created, every instance has its own
	// cloudlet number
	...
	public VideoStreams(String s, String inputdataFolderURL,
			boolean startupqueue, String estimatedGopLength, int seedShift,
			int userId, int videoId, int cloudlets) {
		this.command = s;
	...
	public String call() {
		...
		// System.out.println(Thread.currentThread().getName()+" Start. Video Stream_"+
		// command);
	...
	private void processCommand() {
		...
		/*
		 * Random randomSeed = new Random(seedShift); int shift =
		 * randomSeed.nextInt(1000)+1000;
		 */
		...
		if (fileIndex < listOfFiles.length) {
			...
			// if the file is hidden file, it won't be accepted and move to next
			// one
			while (!accept(file)) {
				fileCount++;
				fileIndex++;
			...
			// cloudletList.clear();
			System.out.println("\n**************************Video Stream_"
					+ command + " just arrived**************************\n");

			System.out.println(Thread.currentThread().getName()
					+ " Video Stream_" + command + ": Created "
					+ cloudletList.size() + " Cloudlets\n");
			...
			synchronized (this) {
				if (startupqueue) {
					...
					if (cloudletList.size() > NEW_ARRIVAL_QUEUE_SIZE) {
						...
						for (int j = NEW_ARRIVAL_QUEUE_SIZE; j < cloudletList
								.size(); j++) {
			...
			/*
			 * System.out.println(Thread.currentThread().getName()
			 * +" Video Stream_"+ command + " Thread is going to sleep " +
			 * sleepTime + "ms."); Thread.sleep(sleepTime);
			 */
		}
	}
	...
	private List<VideoSegment> createCloudlet(int userId, int videoId, File file) {
		...
		for (int i = 0; i < cloudletNum; i++) {
			...
			// length is the execution time mutiply by mips =10000, so it's 10
			// per ms
			// It was runing on Mac whose Mips is 50000, covert it to cloud mips
			// length =
			// pd.getGopTranscodingTimeList().get(i)*10000*(10000/10000);
			if (estimatedGopLength.equals("BEST")) {
				length = (pd.getGopTranscodingTimeList().get(i) - pd
						.getStdList().get(i)) * 8000;
			...
			/*
			 * double val = random.nextGaussian()*std + avglength; length =
			 * (long) Math.round(val);
			 */
			...
			// In case utility function becomes too small
			/*
			 * if(utilityNum <= 0.1){ utilityNum = 0.1; }
			 */

			int orderNum = i;
			
			long estimatedCloudletLength = Long.MAX_VALUE;

			// System.out.println(utilityNum);

			cloudlet[i] = new VideoSegment(i, utilityNum, orderNum, videoId,
					length, estimatedCloudletLength, avglength, std, arrivalTime, deadlineBeforePlay,
					deadlineAfterPlay, pesNumber, fileSize, outputSize,
