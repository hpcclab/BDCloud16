...
public class TranscodingBroker extends DatacenterBroker {
	...
	private static List<VideoSegment> cloudletDropList = new ArrayList<VideoSegment>();
	...
	boolean generatePeriodicEvent = true; // true if new internal events have to
											// be generated

	// All the instance share cloudletNewArrivalQueue and cloudletBatchqueue,
	// both of them are synchronized list
	private static List<VideoSegment> cloudletNewArrivalQueue = Collections
			.synchronizedList(new ArrayList<VideoSegment>());
	private static List<VideoSegment> cloudletBatchQueue = Collections
			.synchronizedList(new ArrayList<VideoSegment>());
	...
	public TranscodingBroker(String name,
			DatacenterCharacteristics characteristics, String propertiesFileURL)
			throws Exception {
		super(name);

		Properties prop = new Properties();
		// InputStream input = new
		// FileInputStream("/Users/lxb200709/Documents/TransCloud/cloudsim/modules/cloudsim-impl/config.properties");
	...
	public void processEvent(SimEvent ev) {
		 ...
		 *           cloudletwaitinglist in VideoSegmentScheduler, whenever a
		 *           vm's waitinglist is smaller than 2, it will add a event in
		 *           the broker, so that broker can keep send this vm the rest
		 *           of cloudlet in its batch queue
		 **/
	...
	public void setVmCost(TranscodingVm vm) {
		vmCost += characteristics.getCostPerSecond()
				* (vm.getVmFinishTime() - vm.getStartTime()) / 1000.0;
	}
		...
		if (vmTempList.size() > 0) {
			...
			for (Vm vm : vmTempList) {
				if (totalCompletionTime_vmMap_temp.containsKey(vm.getId())) {
					totalCompletionTime_vmMap.put(vm.getId(),
							totalCompletionTime_vmMap_temp.get(vm.getId()));
				} else {
	...
	protected void submitCloudlets() {
		...
		/*
		 * if(CloudSim.clock() >= 120000){ System.out.println("Test broker..");
		 * }
		 */
		/**
		 * Now what it should do next: 1. find a calculate each vm's estimated
		 * completion time 2. find the smallest completion time vm, and send it
		 * cloudlet.
		 */
		...
		while (true) {
			...
			for (Map.Entry<Integer, Double> entry : totalCompletionTime_vmMap
					.entrySet()) {
				if (minCompletionTime_vm == null
						|| minCompletionTime_vm.getValue() >= entry.getValue()) {
			...
			// check if this vm is about to be destroyed or not, if yes, find
			// another one
			// check vm's remaining time
			if (vmToSend.getDeallocationFlag()
					&& vmToSend.getRemainingTime() <= 0) {
				 ...
				 * "The mini completion time is: " +
				 * minCompletionTime_vm.getValue());
				 * 
				 * System.out.println(CloudSim.clock() + "VM#: " +
				 * vmToSend.getId() + "'s renting time until " +
				 * vmToSend.getRentingTime() + "...The remaining time is: " +
				 * vmToSend.getRemainingTime());
				 */

				// broker_vm_deallocation_flag = true;
				// getVmList().remove(vmToSend);

				VideoSchedulerSpaceShared vmcsch = (VideoSchedulerSpaceShared) vmToSend
						.getCloudletScheduler();

				if (vmcsch.getCloudletExecList() == null
						&& vmcsch.getCloudletWaitingList() == null
						&& vmcsch.getCloudletPausedList() == null) {
					// System.out.println(CloudSim.clock() +
					// "\n********************VM_" + vmToSend.getId() +
					// "'s renting time is over and to be destroyed***********************"
					// );
					sendNow(getVmsToDatacentersMap().get(vmToSend.getId()),
							CloudSimTags.VM_DESTROY, vmToSend);
				...
				continue;
			} else if (vmToSend.getDeallocationFlag()
					&& vmToSend.getRemainingTime() > 0) {
				/*
				 * System.out.println("\nThe VM#: " + vmToSend.getId()
				 * +" has been set to destroy, but the remaing time is: " +
				 * vmToSend.getRemainingTime() + " So " +
		...
		/*
		 * 1. calculate the minimum completion time when cloudlet_new insert in
		 * the front of batch queue
		 */
		...
		if (startupqueue) {
			...
			if (getCloudletNewList().size() > 0) {
				...
				if (getCloudletList().size() > 0) {
					...
					// check if cloudlet in new arrial and batch are the same
					// video stream
					// If they are the same video, always send the new arrival
					// queue first
					...
					if (cloudlet_new.getVmId() == -1) {
						...
						vm = VmList
								.getById(getVmList(), cloudlet_new.getVmId());
						if (vm == null) {
							// vm was not created
							/*
							 * Log.printLine(CloudSim.clock() + ": " + getName()
							...
							Log.printLine(getName()
									+ ": Postponing execution of cloudlet "
									+ cloudlet_new.getCloudletId()
									+ ": bount VM not available");
							return;
						}
					}
					// Set Each VM CloudletWaitingList to 2, so if a VM's
					// waitingList is beyound 2, current cloudlet
					// won't be sent to specific vm.
					VideoSchedulerSpaceShared vcschTemp = (VideoSchedulerSpaceShared) vm
							.getCloudletScheduler();
					double capacity = 0.0;
					int cpus = 0;
					...
					// reset each cloudlet's deadline after the first cloudlet
					// begins to display
					if (cloudlet_new.getCloudletVideoId() == cloudlet_batch.getCloudletVideoId()) {
						...
						double val = r.nextGaussian()
								* cloudlet_new.getCloudletStd()
								+ cloudlet_new.getAvgCloudletLength();
						...
						//If the cloud miss its deadline, then drop it
						while(cloudlet_new.getCloudletDeadline() < (estimated_completionTime + CloudSim.clock()) && cloudlet_new.getCloudletId() !=0){
							cloudletDropList.add(cloudlet_new);
							getCloudletNewList().remove(cloudlet_new);
							
							System.out.println("<------Video Id: " + cloudlet_new.getCloudletVideoId() + " Cloudlet ID: " + 
									cloudlet_new.getCloudletId() + "is gonna miss its deadline. DROP----->");
							if(getCloudletNewList().size() > 0){
								cloudlet_new = (VideoSegment) getCloudletNewList().get(0);
								estimated_completionTime = cloudlet_new.getCloudletLength() / capacity + minCompletionTime_vm.getValue();
							}else{
						...
						if (!displayStartupTimeMap.containsKey(cloudlet.getCloudletVideoId())|| cloudlet.getCloudletId() == 0) {
							...
							for (VideoSegment vs : cloudletNewList) {
								if (vs.getCloudletVideoId() == cloudlet
										.getCloudletVideoId()) {
									vs.setCloudletDeadline(vs
											.getCloudletDeadlineAfterPlay()
											+ displayStartupTimeMap.get(vs
													.getCloudletVideoId()));
								}
							}
					...
					} else {// if they are not the same video, then continue
							// algorithm

						/*			    *//**
						 * Now what it should do next: 1. find a calculate
						 * each vm's estimated completion time 2. find the
						 * smallest completion time vm, and send it cloudlet.
						 */
						...
						double testTime1 = cloudlet_new.getCloudletDeadline();
						double testTime2 = estimated_completionTime + CloudSim.clock();
						
						
						//If the cloud miss its deadline, then drop it, until find a gop won't miss its deadline
						while(cloudlet_new.getCloudletDeadline() < (estimated_completionTime + CloudSim.clock()) && cloudlet_new.getCloudletId() != 0 ){
							cloudletDropList.add(cloudlet_new);
							getCloudletNewList().remove(cloudlet_new);
						...
						// After calclulate the estimated min completion time of
						// inserted new cloudlet, replace that vm's completion
						// time
						// Create a new map to find the minimum completion time
						// Vm
						...
						for (Integer key : totalCompletionTime_vmMap_New
								.keySet()) {
							if (key == vmIndex) {
						...
						// compare this min completion time with other vm's
						// completion time again in the
						// totalCompletionTime_vmMap
						// Find the min completion time VM afater insert new
						// arrival cloudlet

						Map.Entry<Integer, Double> minCompletionTime_vm_new = null;
						for (Map.Entry<Integer, Double> entry : totalCompletionTime_vmMap_New
								.entrySet()) {
							if (minCompletionTime_vm_new == null
									|| minCompletionTime_vm_new.getValue() > entry
											.getValue()) {
								minCompletionTime_vm_new = entry;
							}
						}

						// double minToCompleteTime =
						// minCompletionTime_vm_new.getValue() +
						// cloudlet_batch.getCloudletLength() / capacity +
						// CloudSim.clock();
						
						
						//If the cloud miss its deadline, then drop it
						
						double estimated_completionTime_batch = cloudlet_batch.getCloudletLength() / capacity + minCompletionTime_vm.getValue();

						while(cloudlet_batch.getCloudletDeadline() < (estimated_completionTime_batch + CloudSim.clock())){
							cloudletDropList.add(cloudlet_batch);
							getCloudletList().remove(cloudlet_batch);
							System.out.println("<------Video Id: " + cloudlet_batch.getCloudletVideoId() + " Cloudlet ID: " + 
									          cloudlet_batch.getCloudletId() + "is gonna miss its deadline. DROP----->");
							if(getCloudletList().size() > 0){
								cloudlet_batch = (VideoSegment) getCloudletList().get(0);
								estimated_completionTime_batch = cloudlet_batch.getCloudletLength() / capacity + minCompletionTime_vm.getValue();
							}else{
								return;
							}
						}
						...
						double minToCompleteTime = minCompletionTime_vm_new.getValue() + cloudlet_batch.getCloudletLength() / capacity
								+ CloudSim.clock();

						// double cloudletDeadlineAbs =
						// cloudlet_batch.getCloudletDeadline() +
						// cloudlet_batch.getSubmissionTime(getId());
						double cloudletDeadlineAbs = cloudlet_batch
								.getCloudletDeadline();
					...
					// If batch queue is empty, but new arrival queue is not,
					// send the new arrival cloudlet
				} else {
					cloudlet = (VideoSegment) getCloudletNewList().get(0);
					switch_flag = 1;
					...
					estimated_completionTime = cloudlet_new.getCloudletLength()
							/ capacity + minCompletionTime_vm.getValue();
					...
					if (!displayStartupTimeMap.containsKey(cloudlet
							.getCloudletVideoId())
							|| cloudlet.getCloudletId() == 0) {
						displayStartupTimeMap.put(
								cloudlet.getCloudletVideoId(),
								estimated_completionTime + CloudSim.clock());
						// cloudlet.setCloudletDeadline(minToCompleteTime);
				...
				if (getCloudletList().size() > 0) {
					...
					if (cloudlet_batch.getVmId() == -1) {
						...
						vm = VmList
								.getById(getVmList(), cloudlet_batch.getVmId());
						if (vm == null) {
							// vm was not created
							/*
							 * Log.printLine(CloudSim.clock() + ": " + getName()
					 ...
					 */
					Random r = new Random();
					double val = r.nextGaussian() * cloudlet_batch.getCloudletStd()
							+ cloudlet_batch.getAvgCloudletLength();
					long sampleLength = (long) Math.round(val);
					cloudlet_batch.setCloudletLength(sampleLength);
					
					estimated_completionTime = cloudlet_batch.getCloudletLength()
							/ capacity + minCompletionTime_vm.getValue();
					
					/*if(cloudlet_batch.getCloudletVideoId() == 14 && cloudlet_batch.getCloudletId() == 5){
						System.out.println("Test...");
					}*/
					
					//If the cloud miss its deadline, then drop it
					while(cloudlet_batch.getCloudletDeadline() < (estimated_completionTime + CloudSim.clock()) ){
						cloudletDropList.add(cloudlet_batch);
						getCloudletList().remove(cloudlet_batch);
						System.out.println("<------Video Id: " + cloudlet_batch.getCloudletVideoId() + " Cloudlet ID: " + 
								          cloudlet_batch.getCloudletId() + "is gonna miss its deadline. DROP----->");
					...
					// if both new arrival and batch queue are empty, return and
					// stop sending cloudlets
				} else {
					return;
				}
			}
			...
			if (getCloudletList().size() > 0) {
				cloudlet_batch = (VideoSegment) getCloudletList().get(0);
				// check if cloudlet in new arrial and batch are the same video
				// stream
				// If they are the same video, always send the new arrival queue
				// first
				...
				//If the cloud miss its deadline, then drop it
				while(cloudlet_batch.getCloudletDeadline() < (estimated_completionTime + CloudSim.clock()) && cloudlet_batch.getCloudletId() !=0){
					cloudletDropList.add(cloudlet_batch);
					getCloudletList().remove(cloudlet_batch);
					System.out.println("<------Video Id: " + cloudlet_batch.getCloudletVideoId() + " Cloudlet ID: " + 
							          cloudlet_batch.getCloudletId() + "is gonna miss its deadline. DROP----->");
				...
				if (!displayStartupTimeMap.containsKey(cloudlet_batch
						.getCloudletVideoId())
						|| cloudlet_batch.getCloudletId() == 0) {
					displayStartupTimeMap.put(
							cloudlet_batch.getCloudletVideoId(),
							estimated_completionTime + CloudSim.clock());
					// cloudlet.setCloudletDeadline(minToCompleteTime);

					for (VideoSegment vs : cloudletList) {
						if (vs.getCloudletVideoId() == cloudlet_batch
								.getCloudletVideoId()) {
							vs.setCloudletDeadline(vs
									.getCloudletDeadlineAfterPlay()
									+ displayStartupTimeMap.get(vs
		...
		if(cloudlet.getCloudletVideoId() == 2){
			System.out.println("test");
		}
		...
		// Set Each VM CloudletWaitingList to 2, so if a VM's waitingList is
		// beyound 2, current cloudlet
		// won't be sent to specific vm.
		VideoSchedulerSpaceShared vcsch = (VideoSchedulerSpaceShared) vm
				.getCloudletScheduler();
		List<? extends ResCloudlet> waitinglist = vcsch
				.getCloudletWaitingList();
		/*
		 * double capacity = 0.0; int cpus = 0; for (Double mips :
		 * vcsch.getCurrentMipsShare()) { capacity += mips; if (mips > 0) {
		 * cpus++; } }
		 * 
		 * int currentCpus = cpus; capacity /= cpus; estimated_completionTime =
		 * cloudlet.getCloudletLength() / capacity +
		 * minCompletionTime_vm.getValue();
		 */
		if (waitinglist.size() >= waitinglist_max) {
			...
			Log.printLine(CloudSim.clock() + getName() + ": Sending Video ID: "
					+ cloudlet.getCloudletVideoId() + " Cloudlet "
					+ cloudlet.getCloudletId() + " to VM #" + vm.getId());
			cloudlet.setVmId(vm.getId());

			EventData data = new EventData(cloudlet, totalCompletionTime_vmMap);
			// totalCompletionTime_vmMap_Min.put(vmIndex,
			// estimated_completionTime);

			sendNow(getVmsToDatacentersMap().get(vm.getId()),
					CloudSimTags.CLOUDLET_SUBMIT, data);
			cloudletsSubmitted++;

			getCloudletSubmittedList().add(cloudlet);

			/*
			 * Recalculate the order number of that video
			 */
	...
	protected void processCloudletReturn(SimEvent ev) {
		 ...
		 * cloudlet.getCloudletId() + " finished in VM" + cloudlet.getVmId());
		 */
		Log.printLine(CloudSim.clock() + getName() + " : Video Id"
				+ cloudlet.getCloudletVideoId() + " Cloudlet "
				+ cloudlet.getCloudletId() + " finished in VM"
				+ cloudlet.getVmId());
		cloudletsSubmitted--;
		...
		TranscodingVm vm = (TranscodingVm) VmList.getById(getVmsCreatedList(),
				cloudlet.getVmId());
		...
		if (!vm.getDeallocationFlag() || vm.getRemainingTime() > 0) {
			// calculate the difference between the real exec time and estimated
			// exec time
			double estimatedExecTime = cloudlet.getCloudletLength() / 8000;
			double execTime_dif = execTime - estimatedExecTime;

			totalCompletionTime_temp = totalCompletionTime_vmMap.get(vmId)
					- execTime + execTime_dif;
			totalCompletionTime_vmMap.put(vmId, totalCompletionTime_temp);
		}
		
		/*
		 * Reestimate each GOP's transcoding time.
		 */
		long totalPreviousCloudletLength = 0;
		long totalPreviousCloudletSize = 0;
		long meanPreviousCloudletSize = 0;
		long meanPreviousCloudletLength = 0;
		long stdPreviousCloudletLength = 0;
		int previousCloudletNum = 0;
		long variance = 0;
		
		//Calculate previous GOPs' average transcoding time and std;
		List<Long> previousCloudletLength = new ArrayList<Long>();
		
		for(Cloudlet cl:getCloudletReceivedList()){
			VideoSegment vs = (VideoSegment)cl;
			if(vs.getCloudletVideoId() == cloudlet.getCloudletVideoId()){
				previousCloudletLength.add(vs.getCloudletLength());
				totalPreviousCloudletLength += vs.getCloudletLength();
				totalPreviousCloudletSize += vs.getCloudletFileSize();
				previousCloudletNum ++;
		...
		meanPreviousCloudletLength = totalPreviousCloudletLength / previousCloudletNum;
		
		for(int i=0; i<previousCloudletLength.size(); i++){
			variance += (Math.pow(previousCloudletLength.get(i) - meanPreviousCloudletLength, 2) / previousCloudletNum);
		}
		
		stdPreviousCloudletLength = (long)Math.sqrt(variance);
		
		//Calculate previous GOPs' average size;
		meanPreviousCloudletSize = totalPreviousCloudletSize / previousCloudletNum;
		
		long estimatedCloudletLength = 0;
		double tempEstimatedCloudletLength = 0.0;
		double ratio = 0.0;
		for (VideoSegment vs : cloudletNewList) {
			if (vs.getCloudletVideoId() == cloudlet.getCloudletVideoId()) {	
				ratio = vs.getCloudletFileSize()*1.0/meanPreviousCloudletSize;
				tempEstimatedCloudletLength = ratio*meanPreviousCloudletLength + stdPreviousCloudletLength/3;	
				estimatedCloudletLength = (long)tempEstimatedCloudletLength;
				vs.setEstimatedCloudletLength(estimatedCloudletLength);
			}
		}
		...
		if (cloudlet.getCloudletId() == 0) {
			...
			videoStartupTime = cloudlet.getFinishTime()
					- cloudlet.getArrivalTime();
			videoStartupTimeMap.put(cloudlet.getCloudletVideoId(),
					videoStartupTime);
		}
		 ...
		 */
		if (!displayStartupTimeRealMap.containsKey(cloudlet
				.getCloudletVideoId()) || cloudlet.getCloudletId() == 0) {
			displayStartupTimeRealMap.put(cloudlet.getCloudletVideoId(),
			...
			for (VideoSegment vs : cloudletNewList) {
				if (vs.getCloudletVideoId() == cloudlet.getCloudletVideoId()) {
					vs.setCloudletDeadline(vs.getCloudletDeadlineAfterPlay()
							+ displayStartupTimeRealMap.get(vs
									.getCloudletVideoId()));
				}
			}
			for (VideoSegment vs : cloudletList) {
			...
			cloudlet.setCloudletDeadline(cloudlet
					.getCloudletDeadlineAfterPlay()
					+ displayStartupTimeRealMap.get(cloudlet
							.getCloudletVideoId()));
		}

		// debug
		/*
		 * if(cloudlet.getCloudletVideoId() == 0 && cloudlet.getCloudletId() >=
		 * 5){ System.out.println("Test cloudlet return"); }
		 */

		// Check if this vm is set to be destroyed
		if (vm.getDeallocationFlag() && vm.getRemainingTime() <= 0) {

			// Before destroying this vm, make sure all the cloudlets in this vm
			// are finished.
			VideoSchedulerSpaceShared scheduler = (VideoSchedulerSpaceShared) vm
					.getCloudletScheduler();
			if (scheduler.getCloudletExecList().size() == 0
					&& scheduler.getCloudletWaitingList().size() == 0
					&& scheduler.getCloudletPausedList().size() == 0) {

				System.out.println(CloudSim.clock()
						+ "\n********************Cloudles in VM_" + vm.getId()
						+ " have finished***********************");
				sendNow(getVmsToDatacentersMap().get(vm.getId()),
		...
		if (getCloudletList().size() == 0 && getCloudletNewList().size() == 0
				&& cloudletsSubmitted == 0 && !generatePeriodicEvent) { // all
																		// cloudlets
																		// executed
			// Log.printLine(CloudSim.clock() + ": " + getName() +
	 ...
	 * @post $none
	 */
	public void submitCloudletList(List<? extends Cloudlet> cloudletBatchQueue,
			List<? extends Cloudlet> cloudletNewArrivalQueue) {
		// Before submit new cloudlet list, delete those who have already been
		// submitted.
		// List<? extends Cloudlet> cloudletNewArrivalQueue_temp =
		// Collections.synchronizedList(new ArrayList<Cloudlet>());
		List<Cloudlet> cloudletBatchQueue_temp = Collections
				.synchronizedList(new ArrayList<Cloudlet>());
		List<Cloudlet> cloudletNewQueue_temp = Collections
				.synchronizedList(new ArrayList<Cloudlet>());
		...
		System.out.println(Thread.currentThread().getName()
				+ "*****New arrival queue Video ID_" + videoId + ": "
				+ newcloudlets + " **********");

		System.out.println("**********************The size of batch queue is: "
	...
	protected void FCFS() {
		...
		for (int a = 0; a < lstCloudlets.size(); a++) {
			for (int b = a + 1; b < lstCloudlets.size(); b++) {
				...
				if (lstCloudlets.get(b).getArrivalTime() < lstCloudlets.get(a)
						.getArrivalTime()) {
				...
				/*
				 * if(lstCloudlets.get(b).getCloudletVideoId() ==
				 * lstCloudlets.get(a).getCloudletVideoId()){ if
				 * (lstCloudlets.get(b).getCloudletDeadline() <
				 * lstCloudlets.get(a).getCloudletDeadline()) { VideoSegment
				 * temp = lstCloudlets.get(a); lstCloudlets.set(a,
				 * lstCloudlets.get(b)); lstCloudlets.set(b, temp); } }else{
				 * 
				 * }
				 */
			}
		 ...
		 * System.out.println("Cloudlet id = " + newcl.getCloudletId() +
		 * " - Length = " + newcl.getCloudletLength());
		 * 
		 * }
		 * 
		 * //Printing ordered list of cloudlets for (Cloudlet cl : lstCloudlets)
	...
	protected void SortedbyDeadline() {
		...
		for (int a = 0; a < lstCloudlets.size(); a++) {
			for (int b = a + 1; b < lstCloudlets.size(); b++) {
				if (lstCloudlets.get(b).getCloudletDeadline() < lstCloudlets
						.get(a).getCloudletDeadline()) {
					VideoSegment temp = lstCloudlets.get(a);
					lstCloudlets.set(a, lstCloudlets.get(b));
					lstCloudlets.set(b, temp);
				}
		 ...
		 * System.out.println("VIDEO ID: " + newcl.getCloudletVideoId() +
		 * " Cloudlet id = " + newcl.getCloudletId() + "   Deadline = " +
		 * newcl.getCloudletDeadline());
		 * 
		 * }
		 * 
		 * System.out.println("\n");
		 ...
		 * System.out.println("VIDEO ID: " + cl.getCloudletVideoId() +
		 * " Cloudlet id = " + cl.getCloudletId() + "   Deadline = " +
		 * cl.getCloudletDeadline()); }
		 */
	}
	...
	protected void SortedbySJF() {
		...
		for (int i = 0; i < reqTasks; i++) {
			executionTimeList.add((lstCloudlets.get(i).getEstimatedCloudletLength())
					/ (lstCloudlets.get(i).getNumberOfPes() * vmList.get(
							i % reqVms).getMips()));
			// System.out.println("CLOUDLET ID" + " "
			// +lstCloudlets.get(i).getCloudletId() +" EXE TIME   " +
			// executionTimeList.get(i));
		...
		/*
		 * System.out.println("After  ordering"); for(int i=0;i<reqTasks;i++) {
		 * 
		 * System.out.println("VIDEO ID: " +
		 * lstCloudlets.get(i).getCloudletVideoId() + " CLOUDLET ID" + " "
		 * +lstCloudlets.get(i).getCloudletId() +" EXE TIME   " +
		 * executionTimeList.get(i)); }
		 */
	...
	public static List<VideoSegment> getCloudletDropList() {
		return cloudletDropList;
	}
